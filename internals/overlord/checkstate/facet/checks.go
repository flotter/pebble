...
const Key plan.PartName = "checks"

func ToChecks(part plan.Part) (Checks, error) {
	facet, ok := part.(*Facet)
	if ok {
		return facet.Checks, nil
	}
	return nil, fmt.Errorf("cannot assert facet type as checks")
}
...
func (f *Facet) Combine(other plan.Part) error {
	...
	for key, entry := range otherFacet.Checks {
		switch entry.Override {
		case plan.MergeOverride:
			if old, ok := f.Checks[key]; ok {
				copied := old.Copy()
				copied.Merge(entry)
				f.Checks[key] = copied
				break
		...
		case plan.ReplaceOverride:
			f.Checks[key] = entry.Copy()
		case plan.UnknownOverride:
			return &plan.FormatError{
				Message: fmt.Sprintf(`Checks must define "override" for entry %q`, key),
			}
		default:
			return &plan.FormatError{
				Message: fmt.Sprintf(`Checks has invalid "override" value for entry %q`, key),
			}
		}
	}
	return nil
...
type Check struct {
	...
	Period    plan.OptionalDuration `yaml:"period,omitempty"`
	Timeout   plan.OptionalDuration `yaml:"timeout,omitempty"`
