...
const Key plan.PartName = "services"

func ToServices(part plan.Part) (Services, error) {
	facet, ok := part.(*Facet)
	if ok {
		return facet.Services, nil
	}
	return nil, fmt.Errorf("cannot assert facet type as services")
}
...
func (f *Facet) Combine(other plan.Part) error {
	...
	for key, entry := range otherFacet.Services {
		switch entry.Override {
		case plan.MergeOverride:
			if old, ok := f.Services[key]; ok {
				copied := old.Copy()
				copied.Merge(entry)
				f.Services[key] = copied
				break
			}
			fallthrough
		case plan.ReplaceOverride:
			f.Services[key] = entry.Copy()
		case plan.UnknownOverride:
			return &plan.FormatError{
				Message: fmt.Sprintf(`Services must define "override" for entry %q`, key),
			}
		default:
			return &plan.FormatError{
				Message: fmt.Sprintf(`Services has invalid "override" value for entry %q`, key),
			}
		}
	}
	return nil
...
type Service struct {
	...
	BackoffDelay   plan.OptionalDuration         `yaml:"backoff-delay,omitempty"`
	BackoffFactor  plan.OptionalFloat            `yaml:"backoff-factor,omitempty"`
	BackoffLimit   plan.OptionalDuration         `yaml:"backoff-limit,omitempty"`
	KillDelay      plan.OptionalDuration         `yaml:"kill-delay,omitempty"`
}
...
func (s *Service) LogsTo(t *logsFacet.LogTargets) bool {
